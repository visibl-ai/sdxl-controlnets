...
ControlnetsInference = modal.Cls.from_name(
    modal_settings.inference_app_id, "ControlnetsInference"
)
...
def run(body: dict):
    ...
    handle = ControlnetsInference().run_batch.spawn(body)
...
async def verify_api_key(
    ...
    CONTROLNETS_API_KEY = settings.api_key

    if not CONTROLNETS_API_KEY:
        logger.error("API key not configured in secrets")
        raise HTTPException(status_code=500, detail="API key not configured")

    if credentials.credentials != CONTROLNETS_API_KEY:
