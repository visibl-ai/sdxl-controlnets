...
from .generate_control_images import generate_canny_map, generate_depth_map
from .image_preprocessing import pre_process_input


def generate_image(
    pipeline, refiner, processed_image, depth_image, canny_image, config, logger
):
...
def process_single_generation(
    pipeline, refiner, depth_estimator, feature_extractor, config, logger
):
    ...
    if not config.local_files_only:
        logger.warning("\n" + "!" * 80)
        logger.warning(
            "local_files_only is set to False - this may cause unnecessary network requests"
        )
        logger.warning(
            "For better performance, set local_files_only: true in config after first run"
        )
        logger.warning("!" * 80 + "\n")
    try:
        # Load input image
        ...
        processed_image, width, height = pre_process_input(
            input_image, target_aspect_ratio=target_aspect_ratio, logger=logger
        )
        ...
        # Generate canny map
        canny_image = generate_canny_map(
            processed_image, config, logger
        ...
        generated_image, refined_image = generate_image(
            pipeline, refiner, processed_image, depth_image, canny_image, config, logger
        )
        ...
        logger.info(
            f"Generation completed in {time.time() - generation_start:.4f} seconds"
        )
