...
class Config:
    ...
    def __init__(self, json_config=None):
        ...
        self.depth_model_type = os.environ.get(
            "DEPTH_MODEL_TYPE", "depth_anything_v2"
        )  # Options: "dpt" or "depth_anything_v2"
        self.depth_model = (
            "Intel/dpt-hybrid-midas"  # Used when depth_model_type is "dpt"
        )
        self.depth_anything_model = os.environ.get(
            "DEPTH_ANYTHING_MODEL", "depth-anything/Depth-Anything-V2-Base-hf"
        )  # Used when depth_model_type is "depth_anything_v2"
    ...
    def get_target_aspect_ratio(self):
        ...
        if self.aspect_ratio and self.aspect_ratio != "auto":
            if self.aspect_ratio in ["square", "landscape", "portrait"]:
                ...
                logger = logging.getLogger(__name__)
                logger.warning(
                    f"Invalid aspect_ratio '{self.aspect_ratio}', using auto-detection"
                )
                return None
