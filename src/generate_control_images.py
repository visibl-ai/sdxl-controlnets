...
def generate_depth_map(image, depth_estimator, feature_extractor, config, logger):
    ...
    if config.depth_model_type == "dpt":
        return generate_dpt_depth_map(
            image, depth_estimator, feature_extractor, config, logger
        )
    elif config.depth_model_type == "depth_anything_v2":
        return generate_depth_anything_v2_map(
            image, depth_estimator, feature_extractor, config, logger
        )
    else:
...
def generate_dpt_depth_map(image, depth_estimator, feature_extractor, config, logger):
    ...
    depth_image = feature_extractor(images=image, return_tensors="pt").pixel_values.to(
        config.device
    )
...
def generate_depth_anything_v2_map(
    image, depth_estimator, feature_extractor, config, logger
):
    ...
    logger.info(
        f"Depth Anything V2 depth map generation took {time.time() - start_time:.4f} seconds"
    )
...
def generate_canny_map(image, config, logger):
    ...
    logger.info(
        f"Canny preprocessing completed in {time.time() - preprocess_start:.2f}s"
    )
    return edges_rgb
