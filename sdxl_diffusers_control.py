...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="Path to JSON configuration file containing array of configurations",
    )
    ...
    try:
        # Load all models
        pipeline, refiner, depth_estimator, feature_extractor = load_models(
            base_config, logger
        )

        # Run inference (batch or interactive mode)
        run_inference(
            args, pipeline, refiner, depth_estimator, feature_extractor, logger
        )
